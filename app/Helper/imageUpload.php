...
class imageUpload
{
    ...
    static function singleUploadUpdate($name, $directory, $file,$data,$field)
    {
        ...
        if (!empty($file)) {
            if (!File::exists($dir)) {
                File::makeDirectory($dir, 0755, true); //yazma izni(0755) verildi.
            }
            if (!File::exists($dirlarge)) {
                File::makeDirectory($dirlarge, 0755, true); //yazma izni(0755) verildi.
            }
            ...
            Image::make($file->getRealPath())->save($path2);
            Image::make($file->getRealPath())->resize(250, 250)->save($path2);
            return $dir . "/" . $filename;
        } else {
            return $data[0][$field];
        }
